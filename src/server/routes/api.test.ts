  ...
  describe("error handler", () => {
    test("SearchError returns 400 JSON", async () => {
      const failRepo: CertificateRepository = {
        ...
        getDailyStats: (from, to) => repo.getDailyStats(from, to),
        computeStatsForPeriod: (start, granularity) => repo.computeStatsForPeriod(start, granularity),
        cleanup: (d) => repo.cleanup(d),
        maintenance: () => repo.maintenance(),
        getMetadata: (key) => repo.getMetadata(key),
        setMetadata: (key, value) => repo.setMetadata(key, value),
